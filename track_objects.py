    ...
    prompt_messages_state = [
        {
            "role": "system",
            "content": [
                "You are a transportation detector. Your task is to count and identify the cars and pedestrians in the provided image.",
            ],
        },
        {
            "role": "user",
            "content": [
                "There are two kinds of objects, cars and pedestrians in the environment. We do not care about any other objects. Some pedestrians might be riding bicycles or motorcycles. Remember to count them in.",
                "Your job is to count how many cars and pedestrians are there in the provided image."
                "Based on the input picture, answer:",
                "1. How many different cars are there in the environment?",
                "2. How many different pedestrians are there in the environment?",
                "You should strictly respond in the format of the following example without any additional information:",
                "Car: 3",
                "Pedestrian: 4",
...
def extract_num_object(response_state):
    ...
    car_match = re.search(r"Car: (\d+)", response_state)
    car_num = int(car_match.group(1)) if car_match else 0

    # extract number of pedestrians
...
def contour_painter(
    ...
    if moments["m00"] != 0 and ann_obj_id is not None:
        ...
        font_scale = 1.2
        font_color = tuple(int(c) for c in color_list[contour_color])
        font_thickness = 3
        cv2.putText(
            painted_image,
            str(ann_obj_id),
...
def main(input_video_path, output_video_path, num_key_frames, bbx_file, index_file):
    ...
    print(object_list_response)
    obj_list = extract_num_object(object_list_response)
    if not obj_list:
    ...
    for i in range(len(frame_names)):
        ...
        for k, mask in video_segments[i].items():
            ...
            #     cv2.putText(img, f"{k}", (avg_x, avg_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, text_color, 2)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_key_frames", type=int, help="Number of key frames")
    parser.add_argument("--bbx_file", type=str, help="Path to store coordinates of bounding boxes")
    parser.add_argument("--index_file", type=str, help="Path to store key frame indexes")

    args = parser.parse_args()

    main(args.input, args.output, args.num_key_frames, args.bbx_file, args.index_file)